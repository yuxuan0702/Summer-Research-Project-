...
link = 'https://raw.githubusercontent.com/yuxuan0702/Summer-Research-Project-/master/final_used_data/update_data.csv'

# import dataset 
df1 = pd.read_csv('update_data.csv')
df1.sort_values('institution',inplace  = True)
print(df1.head(3))

#import dataset 
df2 = pd.read_csv('bardata.csv')
print(df2.head(3))
...
second_card = dbc.Card([
    ...
    dbc.CardBody(
        [
            ...
            html.P("- Economic data is from burearu Statistics"),
            html.P("- Politic data is from a Politic Research ")
        ]
    )],style={'height':'100%'})
...
available_indicators = ['2018 Fall Enrollment',
                        ...
                        'Tuition_out_state_undergraduate']


# bar plot
indicators = ['City', 'Rural', 'Suburb', 'Town', 'No Housing', 'Have Housing',
       'Distant', 'Fringe', 'Large', 'Midsize', 'Remote', 'Small', 'Private',
       'Public', 'two year', 'four year']

# pie 
labels = ['Online', 'TBD', 'In-Person', 'Hybrid']
values = [551, 535, 492, 306]
...
sunburst = px.sunburst(df1,
                       path=['Control', 'Plan', 'college_year'],
                       values='college_year',
                       color='Plan',
...
map_graph = dbc.Card([
    ...
    dbc.CardBody(
    dcc.Graph(id = 'map',figure = usamap,responsive = True)),
   ],style = {'height':'100%'})
